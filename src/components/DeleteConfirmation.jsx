...
function DeleteConfirmation({ onCancel, onDelete, comment, tweet, video }) {
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-80">
            <div className="bg-[#222222] border border-gray-700 rounded-xl p-6 max-w-md w-full mx-4">
                <div className="flex items-start gap-4 mb-4">
                    <ImBin size={28} className="text-red-500 mt-1 flex-shrink-0" />
                    <div>
                        <h1 className="text-xl font-bold mb-2">
                            Delete {`${comment ? "Comment" : ""} ${tweet ? "Tweet" : ""} ${video ? "Video" : ""}`}
                        </h1>
                        <p className="text-gray-400 text-sm">
                            Are you sure you want to delete this {`${comment ? "comment" : ""} ${tweet ? "tweet" : ""} ${video ? "video" : ""}`}?
