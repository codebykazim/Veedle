    ...
    <>
          ...
          {localPlaylists?.map((playlist) => (
            ...
            >
              ...
              <div className="absolute flex justify-between bottom-0 left-0 border-t border-[#1e3a47] py-3 px-4 w-full backdrop-blur-sm bg-[#051622]/40 z-20">
                ...
                <p className="bg-[#051622]/70 px-3 py-1 rounded-md text-xs self-start border border-[#1e3a47]/50">{playlist.totalVideos} Videos</p>
              </div>

              <div className="py-3 px-4 z-20 relative">
                <p className="text-sm font-bold group-hover:text-[#00ed64] transition-colors">{playlist.name}</p>
      ...
      <Dialog open={openCreatePlaylist} onOpenChange={setOpenCreatePlaylist}>
        <DialogContent className="bg-[#072331] border-2 border-[#1e3a47] text-white sm:max-w-md rounded-xl shadow-xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-[#00ed64]">{isEditMode ? "Edit Playlist" : "Create Playlist"}</DialogTitle>
            <button
            ...
            return (
              <div key={key} className="flex justify-between items-center border-b border-[#1e3a47] pb-4 mb-4">
                ...
                <Button
                  ...
                  onClick={() =>
                    isInPlaylist ? handleRemoveVideo(video._id) : handleAddVideo(video._id)
                  }
                >
      ...
      {openDeleteConfirm && (
        <div className="fixed top-0 left-0 w-full h-full bg-[#051622] bg-opacity-90 z-50 flex justify-center items-center p-4">
          <div className="w-full max-w-md bg-[#072331] p-6 rounded-xl border-2 border-[#1e3a47] relative shadow-2xl">
            <button className="absolute top-4 right-4 cursor-pointer text-gray-400 hover:text-white transition-colors" onClick={() => setOpenDeleteConfirm(false)}>
              <X size={24} />
            </button>
            <h2 className="text-2xl mb-4 text-[#00ed64] font-bold">Delete Playlist</h2>
            <p className="mb-6 text-gray-300">Are you sure you want to delete this playlist? This action cannot be undone.</p>

            <div className="flex justify-end gap-3">
              <Button className="bg-[#0d3446] hover:bg-[#164863] border border-[#1e3a47]/50" onClick={() => setOpenDeleteConfirm(false)}>
                Cancel
              </Button>
              <Button className="bg-[#00ed64] hover:bg-[#00c050] text-[#051622] font-medium" onClick={handleDeletePlaylist}>
                Delete
              </Button>
            </div>
          </div>
